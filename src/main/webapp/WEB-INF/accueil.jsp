...
<body>
	...
	<li class='menu'><a href="accueil?action=bouton&bouton=historique" <c:if test="${param.bouton == 'historique'}">
			class="active" </c:if>>Historique</a></li>
	</ul>
	...
	<li class='menu'><a href="accueil?action=bouton&bouton=storyToWrite" <c:if test="${param.bouton == 'storyToWrite'}">
			class="active" </c:if>>Histoire à écrire</a></li>
	...
	<li class='menu'><a href="accueil?action=bouton&bouton=histoireDepubliable" <c:if
			test="${param.bouton == 'histoireDepubliable'}"> class="active" </c:if>>Histoires dépubliables</a></li>
	<li class='menu'><a href="accueil?action=bouton&bouton=historique" <c:if test="${param.bouton == 'historique'}">
			class="active" </c:if>>Historique</a></li>
	...
	<c:if test="${param.bouton == 'createStory'}">
		<c:if test="${paragEnCours != null}">
			<div class='alreadyWritting'>Vous avez déja un paragraphe en cours de rédaction : <a
					href="write_paragraph?idHist=${paragEnCours.idHist}&numParag=${paragEnCours.numParag}&titreParag=${paragEnCours.titre}"
					class='alreadyWritting'>${paragEnCours.titre}</a></div>
		</c:if>
		<c:if test="${paragEnCours == null}">
			<form method="post" id="formCreate" action="createStory" accept-charset="UTF-8">
				<p>
					...
					Confidentialité de l'histoire : <label><input type="radio" onclick="hideInvite();" name="confident"
							checked="checked" id="buttonPublic" value="0" />Publique</label>
					<label><input type="radio" onclick="displayInvite();" name="confident" value="1" />Privée</label>
					<br>
					...
					<br>
					<p>Mon paragraphe est une conclusion :</p> <label><input type="radio" onclick="hideChoice();"
							name="isConclusion" value='1' />Oui</label>
					<label><input type="radio" onclick="displayChoice();" name="isConclusion" checked="checked"
							value='0' />Non</label> <br>
					<div id="listeDesChoix">
						<p>Nombre de choix :</p><input type="number" id="nbChoix" name="nbChoix" value="1" min="1"
							max="100" required>
						<input type="button" value="Afficher les choix" onclick="changeChoice();">
						<br>
						<table id="choice" class="formulaire">
							<tr class="formulaire">
								<td class="formulaire"><input type="text" id="choix1" name="choix1"
										value="Choix numéro 1" required />
								</td>
							</tr>
			...
			<c:forEach items="${entry.value}" var="parag">
				<div class='paragraphStory'>
					...
					<div id="divBouton">
						...
						<a class="boutonParagRedige"
							href="write_paragraph?action=erase&idHist=${parag.idHist}&numParag=${parag.numParag}">Supprimer</a>
					</div>
				</div>
			</c:forEach>
		</c:forEach>
			...
			<c:if test="${not empty histoires}">
				<table>
					...
					<c:forEach items="${histoires}" var="histoire">
						<tr class="activable">
							<td class="click"><a href="write_story?idHist=${histoire.id}&numParagPere=1"
									class="story">${histoire.titre}</a></td>
						</tr>
					</c:forEach>
				</table>
			</c:if>
		...
		<c:if test="${not empty histoires}">
			<table>
				...
				<c:forEach items="${histoires}" var="histoire">
					<tr class="activable">
						<td class="click"><a
								href="read_story?idHist=${histoire.id}&numParagPere=1">${histoire.titre}</a></td>
					</tr>
				</c:forEach>
			</table>
		</c:if>
		...
		<c:if test="${not empty histoiresAPublier}">
			<form method="post" class="formValidHistoire" action="publication">
				...
				<c:forEach items="${histoiresAPublier}" var="histoire">
					<p class="histoirePubli">${histoire.titre} <input type="checkbox" id="${histoire.id}"
							name="${histoire.id}"></p><br>
				</c:forEach>
