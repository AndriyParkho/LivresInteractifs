...
<body>
	...
	<c:if test="${choixParagSuite.size() gt 0}">
		...
		<c:forEach items="${choixParagSuite}" var="choix" varStatus="vs">
			<div class="choixSuite"><a class="choixSuite"
					href="write_story?idHist=${choix.idHist}&choix=${vs.index}&numParagPere=${currentParag.numParag}"
	...
	<c:if test="${choixParagAEcrire.size() gt 0}">
		...
		<c:forEach items="${choixParagAEcrire}" var="choix">
			<div class="choixSuite"><a class="choixSuite"
					href="write_paragraph?&idHist=${choix.idHist}&numParag=${choix.numParag}&titreParag=${choix.titre}"
					class="active">${choix.titre}</a></div>
			<br>
		</c:forEach>
	</c:if>
