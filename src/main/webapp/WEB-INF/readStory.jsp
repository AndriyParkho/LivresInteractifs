...
<body>
	...
	<c:if test="${user == null}">
		<ul class='menu'>
			...
			<li class='menu'><a href="accueil?action=bouton&bouton=historique" <c:if
					test="${param.bouton == 'historique'}"> class="active"
	</c:if>>Historique</a></li>
	</ul>
	...
	<input type="button" id='changeBouton' onclick="changeDisplay();" <c:if test="${StoryFirst}"> value="Historique"
	</c:if>>
	<div id='StoryBox'>
		...
		<c:forEach items="${choixParag}" var="choix" varStatus="vs">
			<div class="choixSuite"><a class="choixSuite"
					href="read_story?idHist=${choix.idHist}&choix=${vs.index}&numParagPere=${current.numParag}"
					class="active">${choix.titre}</a></div>
			<br>
		</c:forEach>
	</div>
	<div id='HistoricBox'>
			...
			<div class="choixHistorique">
				<c:forEach items="${block}" var="choix" varStatus="varstat2">
					<c:if test="${varstat2.index == 0}">
						<a class="TitreHistorique" onclick="goBack(${choix.idHist}, ${varstat.index});"
							class="active">${choix.titre}</a>
					</c:if>
					<p>${choix.texte}</p>
				</c:forEach>
			</div>
